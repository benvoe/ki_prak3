...
function knowledge_base:TELL( sequence )
  ...
  if( self.resolution ) then
    for _,yx in pairs( n ) do 
      if yx then
        if not breeze then self:Insert{ breeze_yx, "-P"..yx } end
        if not stench then self:Insert{ stench_yx, "-W"..yx } end
        
        if stench then table.insert( wump_clausel, "W"..yx ) end
        if breeze then table.insert( pit_clausel, "P"..yx ) end
      end
    end
    
    if stench then self:Insert( wump_clausel ) end
    if breeze then self:Insert( pit_clausel ) end
  else
    for i,yx in pairs( n ) do
      
      if yx then
        table.insert( self.kb, { Not(stench_yx).."=>-W"..yx } )
        table.insert( self.kb, { Not(breeze_yx).."=>-P"..yx } )
      end
    ...
    if inferred[ p ] == false then 
      ...
      for i,clause in ipairs( kb ) do
        if self:clause_contains( clause, p ) then
          count[i] = count[i] - 1
          if count[i] == 0 then 
            agenda:push( self:conclusion( clause ) )
          end
        end
      end
    end
...
function knowledge_base:clause_contains( clause, p )
...
function knowledge_base:conclusion( clause ) 
  local cc =  toChar( clause[ #clause ] )
  local conclusion = ""
  local conc = false
  for i, v in ipairs( cc ) do
    if v == ">" then conc = true
    elseif conc then conclusion = conclusion .. v end
  end
  return conclusion
end
